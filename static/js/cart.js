  ...
  return {
    ...
    async fetchProduct(userId) {
      const res = await fetch(
        `${location.origin}/layspgiohang/${userId}/0/5`
      );
      const data = await res.json();
      return data;
    },
                ...
                <div>
                    <div class=" flex justify-between text-base font-medium text-gray-900 gap-4">
                        <h3 class="line-clamp-2 hover:text-[#00c16a]">
                            <a href="/chitietsanpham/${item.spid}">${
            item.ten
          }</a>
                    ...
                    <span class="text-gray-500">${item.mausac}</span> | 
                    <span class="text-gray-500">${item.kichco}</span>
                    </div>
                    <p class="text-gray-500">X ${item.soluongg}</p>
                </div>
            </div>
        </li>
      `;
    ...
    async init() {
      // load
      this.fetchQuantity(userId).then(res=>{
        quantity.innerHTML = parseInt(res.sl) > 99 ? "99+" : res.sl;
      }).catch((err)=>{
      ...
      btn?.addEventListener("click", async () => {
        ...
        if (isChange) {
          ...
          this.fetchProduct(userId)
            .then((data) => {
              if (data.length != 0) {
                str = this.renderData(data['spgiohang']);
                cart.innerHTML = str;
              } else {
                cart.innerHTML = "";
              }
