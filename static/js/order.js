  ...
  return {
    ...
    async getOrders(type) {
      const res = await fetch(
        `${location.origin}/donhang/${type}/`
      );
      return await res.json();
    },
    async cancelOrder(id) {
      const res = await fetch(`${location.origin}/huydonhang/`, {
        method: "POST",
        headers: {
      ...
      if (data.length === 0) {
        ...
        <div class="h-80 flex items-center justify-center py-10 bg-white">
            <div class="flex flex-col items-center gap-4">
                <img class="aspect-square" src="/static/images/none.png" alt="image"><span class="text-gray-900">Chưa có đơn hàng</span>
            </div>
        </div>
        `;
      } else {
    ...
    showModalComment(str) {
      ...
      modalComment.querySelector(".modal-comment-img").src = `/media/${data.chitietdonhang[0].hinhanh}`;
