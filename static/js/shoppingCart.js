    ...
    return {
      async getProducts(page, limit) {
        const res = await fetch(
          `${location.origin}/layspgiohang/${userId}/${(page - 1) * limit}/${limit * page}`
        );
          ...
          elmContent.innerHTML = `
            ...
            item.soluongg
              ...
              <span data-kcspid-${item.kcspid} class="text-center w-1/5 font-semibold text-rose-500 text-sm">${this.formatPrice(
                parseFloat(item.gia) * parseInt(item.soluongg)
              )}</span>
      ...
      async updateQt(id, type, qt) {
        const res = await fetch(`${location.origin}/giohangnguoidung/`, {
          method: "POST",
          headers: {
      ...
      async fetchQuantity(userId) {
        const res = await fetch(`${location.origin}/layslspgiohang/`);
        const data = await res.json();
        return data;
      },
      init() {
          ...
          if (e.target.tagName == "INPUT" && e.target.name === "sps[]") {
            ...
            sum.innerHTML = this.formatPrice(
              productChecked.reduce((prev, curv) => {
                return prev + curv.gia * curv.soluongg;
              }, 0)
            );
          } else if (
            ...
            if (e.target.dataset.increaseQt || e.target.dataset.decreaseQt) {
              ...
              this.updateQt(
                ...
                productItem.soluongg
              )
                .then((res) => {
                  if (JSON.parse(res.data)) {
                    data.type === "decrease"
                      ? (productItem.soluongg = productItem.soluongg - 1)
                      : (productItem.soluongg = productItem.soluongg + 1);
                    elm.value = productItem.soluongg;
                    ...
                    if(itemGh){
                      document.querySelector(`[data-kcspid-${productItem.kcspid}]`).innerHTML = this.formatPrice(parseFloat(itemGh.gia) * parseFloat(itemGh.soluongg));
                    }
        ...
        form.addEventListener("submit", (e) => {
          ...
          if (productChecked.length === 0) {
            ...
            let arr = [];
            document.querySelectorAll("input[name='sps[]']").forEach(function(elem) {
              if(elem.checked){
                arr.push(elem.value)
              }
            });
            document.querySelector("input[name='dsspck']").value=JSON.stringify(arr);
            // console.log(document.querySelector("input[name='dsspck']").value)
