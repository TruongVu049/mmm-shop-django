const user = (function () {
  ...
  return {
    ...
    async fetchDataPost(
      ...
      const res = await fetch(
        `${location.origin}/diachinguoidung/${userId ?? 0}/`,
        {
          method: "POST",
          headers: {
    ...
    async getList(id) {
      const res = await fetch(`${location.origin}/diachinguoidung/${userId.value ?? 0}`);
      return await res.json();
    },
    showList(data) {
      list.innerHTML = "";
    ...
    loadlist() {
      this.getList(userId.value ?? 0).then((res)=>{
        if(res){
          console.log(res.dsdiachi)
          arrAddress = res['dsdiachi'];
    ...
    fillToForm(address) {
      ...
      ipSetDault.checked = JSON.parse(address["macdinh"])
    ...
    async removeAddress(id) {
      const res = await fetch(
        `${location.origin}/diachinguoidung/${userId.value ?? 0}/`, 
        {
        method: "POST",
        headers: {
          "Content-Type": "application/json; charset=utf-8",
